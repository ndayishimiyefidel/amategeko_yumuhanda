...
class _NewQuizState extends State<NewQuiz> {
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return ListView.builder(
            ...
            itemCount: exams.length + 1, // +1 for bottom padding
  ...
  String _buildRequestMessage() {
    return "Hello, my name is $currentUsername and my phone number is $phone. I have paid 1500 Rwf to 0788659575 for the exam. I need the access code. Thank you.";
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildInstructionStep(
                      1,
                      "Pay 1500 Rwf to 0788659575/0728877442 or use the green button below",
                      Colors.blue,
                    ),
                    _buildInstructionStep(
                      2,
...
class _ModernQuizCardState extends State<ModernQuizCard> {
  ...
  Future<void> handleButtonClick() async {
    ...
    try {
      if (widget.quizType == "Free" || widget.userRole == "Admin") {
        ...
        String sameUserLoggedIn =
            preferences.getString("sameUserLoggedIn") ?? "22";
        print("isAlreadyOpened: $isAlreadyOpened");
        print("sameUserLoggedIn: $sameUserLoggedIn");

        if (isAlreadyOpened && sameUserLoggedIn == widget.currentUserId) {
          print("isAlreadyOpened: $isAlreadyOpened");
          print("sameUserLoggedIn: $sameUserLoggedIn");
      ...
      if (data['isOpen'] == true) {
        ...
        try {
          ...
          if (endTimeMillis >= DateTime.now().millisecondsSinceEpoch) {
            ...
            await preferences.setString("sameUserLoggedIn", data['userId']);
            navigateToQuiz();
          } else {
            bool wasAlreadyOpened = await preferences.setBool("isOpened", true);
            String wasSameUser = await preferences
                .setString("sameUserLoggedIn", data['userId'])
                .toString();

            if (!mounted) return;
            setState(() {
              isAlreadyOpened = wasAlreadyOpened;
              wasSameUser = wasSameUser;
            });
            showErrorDialog(
                "Your study period has expired. Please pay again to continue studying.");
          }
  ...
  void navigateToQuiz() {
    ...
    // Navigate to quiz after disclaimer is acknowledged
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) {
